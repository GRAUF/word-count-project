...
const countWords = (paragraph: string): { [key: string]: number } => {
...
const displayTable = (wordCount: { [key: string]: number }): void => {
...
const main = async (): Promise<void> => {
    const { paragraph }: { paragraph: string } = await inquirer.prompt([
        {
            type: 'input',
            name: 'paragraph',
